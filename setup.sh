...
npm's global directory has wrong permissions:
  ...
  # Always .cjs, never .js. PM2 config is CommonJS, but a package.json with
  # "type": "module" makes Node treat every .js file as an ES module — and
  # `module.exports` then throws "module is not defined in ES module scope".
  # The .cjs extension is CommonJS regardless of what "type" says.
  local eco="$APP_DIR/ecosystem.config.cjs"

  # Clean up a .js config from an older run, or PM2 may pick up the stale one.
  if [ -f "$APP_DIR/ecosystem.config.js" ]; then
    if grep -q 'Generated by nextdeploy' "$APP_DIR/ecosystem.config.js" 2>/dev/null; then
      rm -f "$APP_DIR/ecosystem.config.js"
      dim "  (removed the older ecosystem.config.js we wrote previously)"
    else
      warn "Leaving your existing ecosystem.config.js alone; writing .cjs instead."
    fi
  fi
...
//
// .cjs, not .js: this file is CommonJS, and a package.json with
// "type": "module" would otherwise make Node parse it as an ES module.
module.exports = {
  apps: [{
    name: '$APP_NAME',
    cwd: '$APP_DIR',
...
ECOCONF
  ...
  if [ -f "$APP_DIR/.gitignore" ] && ! grep -qE '^ecosystem\.config\.cjs$' "$APP_DIR/.gitignore" 2>/dev/null; then
    echo "ecosystem.config.cjs" >> "$APP_DIR/.gitignore"
  fi

  # Validate the config with Node before handing it to PM2. PM2's own
  # complaint for a bad config is just "malformated", which buries the
  # actual parse error; Node prints the real one.
  if ! node -e "require('$eco')" 2>/dev/null; then
    blank
    err "The generated PM2 config isn't loadable by Node."
    blank
    dim "─── what Node says ───"
    node -e "require('$eco')" 2>&1 | head -n 12 >&2 || true
    dim "──────────────────────"
    hint "This is a bug in nextdeploy, not in your app. Please report it
with the output above:

    https://github.com/rajjayavant/nextdeploy/issues"
    return 1
  fi
  ...
  if [ "$up" -ne 1 ]; then
    ...
    # `pm2 logs` prints only a "[TAILING]" banner when the process never
    # started, which reads as "no errors" when it actually means "nothing
    # ran". Read the log files directly and say when they're empty.
    local logs=""
    logs="$(pm2 logs "$APP_NAME" --lines 30 --nostream 2>/dev/null \
      | grep -v '^\[TAILING\]' | grep -v '^$' || true)"
    if [ -z "$logs" ]; then
      logs="$(tail -n 30 "$HOME/.pm2/logs/$APP_NAME-error.log" 2>/dev/null || true)"
    fi
    ...
    if [ -n "$logs" ]; then
      ...
      dim "  (PM2 has no logs for '$APP_NAME' — the process never started,"
      dim "   so the failure is in how it was launched, not in your app.)"
    fi
    blank

    # Report what PM2 thinks the state is; "errored" vs "online" points at
    # very different causes.
    local pm2_status
    pm2_status="$(pm2 jlist 2>/dev/null \
      | node -e "try{const l=JSON.parse(require('fs').readFileSync(0,'utf8'));const a=l.find(p=>p.name==='$APP_NAME');console.log(a?a.pm2_env.status+' (restarts: '+a.pm2_env.restart_time+')':'not running')}catch(e){console.log('unknown')}" 2>/dev/null || echo "unknown")"
    dim "  PM2 status: $pm2_status"
    blank

    hint "Most common causes:
  • The app crashed on startup — a missing env var is the usual
    culprit. The logs above normally name it.
  • The app is hard-coded to a different port. Check the start
    script in package.json for an explicit -p flag.
  • It binds to localhost only, or is slow to boot. Retry once.

To dig in yourself:

    pm2 logs $APP_NAME --lines 50
    cd $APP_DIR && $PKG_MANAGER run start"
    return 1
  fi
